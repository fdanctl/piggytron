	...
	rows, err := s.db.QueryContext(
		...
		WITH
		  ...
		  cat_value AS (
		    ...
		    FROM
		      cat c
		      LEFT JOIN ledger t ON t.type != 'goal-fulfillment'
			  AND (
				c.id = t.income_category_id
